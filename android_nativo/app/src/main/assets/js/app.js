...
class GameState {
    constructor() {
        ...
        this.audioEnabled = true; // Música habilitada por defecto
        this.soundEnabled = true; // Efectos de sonido habilitados por defecto
    ...
    saveProgress() {
        ...
        // Sincronizar con Firebase SOLO si hay usuario logueado con nick
        const userData = localStorage.getItem('user');
        if (userData && window.AndroidInterface && window.AndroidInterface.saveProgress) {
            try {
                const user = JSON.parse(userData);
                // Solo guardar en Firebase si tiene nick (está logueado, no es invitado)
                if (user.hasNick && user.nick) {
                    ...
                    window.AndroidInterface.saveProgress(JSON.stringify(firebaseProgress));
                    console.log('💾 Progreso guardado en Firebase (usuario logueado)');
                } else {
                    console.log('💾 Progreso guardado solo en localStorage (modo invitado)');
                }
            } catch (e) {
                console.error('Error verificando usuario para sincronización:', e);
            }
        } else {
            console.log('💾 Progreso guardado solo en localStorage (modo invitado)');
        }
    ...
    loadProgress() {
        ...
        if (saved) {
            try {
                ...
                this.soundEnabled = progress.soundEnabled !== undefined ? progress.soundEnabled : true;
            } catch (e) {
                console.error('Error cargando progreso:', e);
            }
        }
        
        // Verificar si hay datos de usuario en localStorage sin nick válido
        const userData = localStorage.getItem('user');
        if (userData) {
            try {
                const user = JSON.parse(userData);
                // Si no tiene nick o el nick está vacío, borrar datos
                if (!user.hasNick || !user.nick || user.nick.trim() === '') {
                    console.log('⚠️ Usuario en localStorage sin nick válido - borrando datos');
                    localStorage.removeItem('user');
                    localStorage.removeItem('rinconesdelmundo_user');
                }
            } catch (e) {
                console.error('Error verificando usuario en localStorage:', e);
                localStorage.removeItem('user');
                localStorage.removeItem('rinconesdelmundo_user');
            }
        }
    ...
    completeLevel(world, level) {
        ...
        // Actualizar mundo y nivel actual
        this.currentWorld = world;
        this.currentLevel = level;
        
        // Guardar progreso (automáticamente sincroniza con Firebase)
        this.saveProgress();
    }
...
class RinconesDelMundo {
        ...
        try {
            ...
            this.puzzlesConfig = {
                worlds: {
                    ...
                    mundo11: { name: "Mundo 11", emoji: "🌸", colorPalette: { background: "#FF69B4", cardBack: "#FFF0F5", cardBorder: "#FF69B4", primary: "#FF69B4" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo12: { name: "Mundo 12", emoji: "🏝️", colorPalette: { background: "#00CED1", cardBack: "#E0FFFF", cardBorder: "#00CED1", primary: "#00CED1" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo13: { name: "Mundo 13", emoji: "🌺", colorPalette: { background: "#FF1493", cardBack: "#FFE4E1", cardBorder: "#FF1493", primary: "#FF1493" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo14: { name: "Mundo 14", emoji: "🏔️", colorPalette: { background: "#708090", cardBack: "#F0F8FF", cardBorder: "#708090", primary: "#708090" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo15: { name: "Mundo 15", emoji: "🌲", colorPalette: { background: "#2E8B57", cardBack: "#F0FFF0", cardBorder: "#2E8B57", primary: "#2E8B57" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo16: { name: "Mundo 16", emoji: "🌵", colorPalette: { background: "#9ACD32", cardBack: "#F5F5DC", cardBorder: "#9ACD32", primary: "#9ACD32" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo17: { name: "Mundo 17", emoji: "🌙", colorPalette: { background: "#191970", cardBack: "#F0F8FF", cardBorder: "#191970", primary: "#191970" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo18: { name: "Mundo 18", emoji: "☀️", colorPalette: { background: "#FFD700", cardBack: "#FFFACD", cardBorder: "#FFD700", primary: "#FFD700" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo19: { name: "Mundo 19", emoji: "⭐", colorPalette: { background: "#FFA500", cardBack: "#FFEFD5", cardBorder: "#FFA500", primary: "#FFA500" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo20: { name: "Mundo 20", emoji: "🎨", colorPalette: { background: "#BA55D3", cardBack: "#E6E6FA", cardBorder: "#BA55D3", primary: "#BA55D3" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo21: { name: "Mundo 21", emoji: "🌈", colorPalette: { background: "#9370DB", cardBack: "#F8F8FF", cardBorder: "#9370DB", primary: "#9370DB" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) },
                    mundo22: { name: "Mundo 22", emoji: "✨", colorPalette: { background: "#48D1CC", cardBack: "#F0FFFF", cardBorder: "#48D1CC", primary: "#48D1CC" }, puzzles: Array.from({length: 15}, (_, i) => ({ name: `Puzzle ${i + 1}`, pieces: 4 + i })) }
                }
            };
    ...
    playSound(soundName) {
        // Verificar soundEnabled solo para efectos de sonido (move y complete)
    ...
    updateAudioSettings() {
        // Aplicar configuración de música solo al background
        if (this.audio.background) {
            // Música: pausar si está desactivada, reproducir si está activada
            if (!this.state.audioEnabled) {
                this.audio.background.pause();
            } else if (this.audioActivated) {
        ...
        // Aplicar configuración de efectos de sonido (move y complete)
        if (this.audio.move) {
            this.audio.move.muted = !this.state.soundEnabled;
            if (!this.state.soundEnabled) {
                this.audio.move.volume = 0;
            } else {
                this.audio.move.volume = 0.3;
            }
        }
        if (this.audio.complete) {
            this.audio.complete.muted = !this.state.soundEnabled;
            if (!this.state.soundEnabled) {
                this.audio.complete.volume = 0;
            } else {
                this.audio.complete.volume = 0.7;
            }
        }
    ...
    // ===== AUTENTICACIÓN =====
    async checkAuth() {
        // Verificar si AndroidInterface está disponible (modo Android con Firebase)
        if (window.AndroidInterface) {
            try {
                // Solicitar datos del usuario al iniciar
                window.AndroidInterface.getUser();
                console.log('✅ Verificación de autenticación solicitada');
            } catch (error) {
                console.warn('⚠️ Error verificando autenticación:', error);
                this.state.isLoggedIn = false;
            }
        } else {
            // Modo web/offline - sin autenticación
            console.log('🌐 Modo web - sin autenticación');
            this.state.isLoggedIn = false;
        }
    }
    ...
    showScreen(screenName) {
        ...
        // Ocultar/mostrar menú de usuario según la pantalla
        const userMenu = document.getElementById('user-menu');
        const topMenuBar = document.getElementById('top-menu-bar');
        if (screenName === 'intro') {
            // Ocultar menús en la pantalla de intro
            if (userMenu) userMenu.style.display = 'none';
            if (topMenuBar) topMenuBar.style.display = 'none';
        } else {
            // Mostrar menús en otras pantallas
    ...
    handleStartup() {
        // Siempre mostrar la pantalla de intro primero
        this.showIntroScreen();
    }
    ...
    updateSettingsUI() {
        // Actualizar sección de cuenta según si está logueado
        if (typeof updateSettingsUI === 'function') {
            updateSettingsUI();
        }
        
        // Actualizar toggles según estado actual
        const musicToggle = document.getElementById('music-toggle');
        const soundToggle = document.getElementById('sound-toggle');
        
        // Toggle de MÚSICA: usar estado audioEnabled
        if (musicToggle) {
            const musicEnabled = this.state.audioEnabled !== undefined ? this.state.audioEnabled : 
                                (this.audio.background && !this.audio.background.paused && !this.audio.background.muted);
            if (musicEnabled) {
        ...
        // Toggle de EFECTOS: usar estado soundEnabled
        if (soundToggle) {
            const soundsEnabled = this.state.soundEnabled !== undefined ? this.state.soundEnabled : 
                                 (this.audio.move && !this.audio.move.muted);
            if (soundsEnabled) {
    ...
    setupSettingsEvents() {
        ...
        if (closeBtn) {
            ...
            newCloseBtn.addEventListener('click', () => {
                console.log('✕ Botón cerrar settings presionado');
                // Volver a la pantalla anterior (intro, worlds o levels)
                if (this.currentScreen === 'intro') {
                    this.showIntroScreen();
                } else if (this.state.currentWorld && this.currentScreen === 'levels') {
                    this.showLevelsScreen(this.state.currentWorld);
                } else {
                    this.showWorldsScreen();
                }
    ...
    setupIntroEvents() {
        // Actualizar contador de puzzles completados
        const completedPuzzlesEl = document.getElementById('intro-puzzles');
    ...
    renderWorlds() {
        ...
        // Usar el número real de mundos cargados del JSON
        const totalWorldsLoaded = Object.keys(this.worldsData).length;
        for (let world = 1; world <= totalWorldsLoaded; world++) {
